...
SELECT employees.employee_id,employees.first_name,employees.last_name,employees.salary,departments.department_name,JOBS.job_title,locations.state_province,countries.country_name,Regions.Region_name,
departments.department_name AS DEPENDENT departments.RELATIONSHIP
FROM employees
JOIN departments ON employees.department_id=departments.department_id
join locations on departments.location_id=locations.location_id
join countries on locations.counrty_id= countries.counrty_id
join jobs on employees.job_id=jobs.job_id
left join departments on employees.employee_id=dpt.employee_id
...
creat  view containting the date of all dependant of null thisi IT department 
EMPID FNAME PHONENUMBER DEPTNAME DEPANDENTNAME RELATIONSHIP
...
select*from locations
select*from dependents
CREATE VIEW IT_Department_No_Dependents 
AS
SELECT 
    Employees.employee_id, Employees.first_name,mployees.phone_number,departments.first_name,dependents.dependent_name,
    dependents.date_of_birth
FROM employees employees
JOIN departments departments ON e.dep_id= departments.department_id
JOIN dependents dependents  ON employees.employee_id = emp_id
...
create view v_ITDEPENDET
AS
SELECT first_name phonename department_name dependent_name relationshep
from employees
inner join department ON emplotee.depatrment_id=deprtment,department_id
inner join depandante ON employee.depandante_id = depandante. depandante_id
where depatment_name ='IT';
